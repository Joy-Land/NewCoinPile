# Package Starter Kit
- 提供Evolution开发套件最基础的功能组件，比如统一外部读写目录环境；
- 各Evolution模块都共同依赖的组件，比如文件操作
...
<root>
  ...
  ├── Editor
  │   ├── topjoy.evolution.EvoCore.Editor.asmdef
  │   └── EditorExample.cs
  ├── Runtime
  │   ├── topjoy.evolution.EvoCore.asmdef
  │   └── RuntimeExample.cs
  ├── Tests
  │   ├── .tests.json
  │   ├── Editor
  │   │   ├── topjoy.evolution.EvoCore.Editor.Tests.asmdef
  │   │   └── EditorExampleTest.cs
  │   └── Runtime
  │        ├── topjoy.evolution.EvoCore.Tests.asmdef
...
1. Enter your package name. The name you choose should contain your default organization followed by the name you typed. For example: `topjoy.evolution.EvoCore`.
...
To use the Inspector, select the `package.json` file in the Project browser. The **Package EvoCore Manifest** page opens for editing.
...
* Store Editor-specific code under a root editor assembly definition file:
  ...
  `Editor/topjoy.evolution.EvoCore.Editor.asmdef`

* Store runtime-specific code under a root runtime assembly definition file:

  `Runtime/topjoy.evolution.EvoCore.asmdef`

* Configure related test assemblies for your editor and runtime scripts:

  `Tests/Editor/topjoy.evolution.EvoCore.Editor.Tests.asmdef`

  `Tests/Runtime/topjoy.evolution.EvoCore.Tests.asmdef`
...
Use the `Documentations~/EvoCore.md` documentation file to create preliminary, high-level documentation. This document should introduce users to the features and sample files included in your package.  Your package documentation files will be used to generate online and local docs, available from the Package Manager UI.
