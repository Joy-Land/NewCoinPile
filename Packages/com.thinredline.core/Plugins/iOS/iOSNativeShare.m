...
void shareTextToWhatsapp(const char *msg) {
    ...
    NSString * urlWhats = [NSString stringWithFormat:@"whatsapp://send?text=%@",nsMsg];
...
bool checkAppInstalled(const char *appName) {
    NSString *nsName = [[NSString alloc] initWithUTF8String:appName];
    NSString * appScheme = [NSString stringWithFormat:@"%@://app",nsName];
    NSURL * appURL = [NSURL URLWithString:[appScheme stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
