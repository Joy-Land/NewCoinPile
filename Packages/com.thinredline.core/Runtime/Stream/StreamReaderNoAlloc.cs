/*
功能描述：
    本模块用于维护StreamReaderNoAlloc，用尽量少的触发GC的方式，逐行处理文件
*/
...
namespace ThinRL.Core.FileSystem
{
    ...
    public class StreamReaderNoAlloc : TextReader
    {
        // StreamReader.Null is threadsafe.
        public new static readonly StreamReaderNoAlloc Null = new NullStreamReader();
        ...
        internal StreamReaderNoAlloc()
        {
        }

        public StreamReaderNoAlloc(Stream stream)
            : this(stream, true)
        {
        }

        public StreamReaderNoAlloc(Stream stream, bool detectEncodingFromByteOrderMarks)
            : this(stream, Encoding.UTF8, detectEncodingFromByteOrderMarks, DefaultBufferSize, false)
        {
        }

        public StreamReaderNoAlloc(Stream stream, Encoding encoding)
            : this(stream, encoding, true, DefaultBufferSize, false)
        {
        }

        public StreamReaderNoAlloc(Stream stream, Encoding encoding, bool detectEncodingFromByteOrderMarks)
        ...
        // 
        public StreamReaderNoAlloc(Stream stream, Encoding encoding, bool detectEncodingFromByteOrderMarks, int bufferSize)
            : this(stream, encoding, detectEncodingFromByteOrderMarks, bufferSize, false)
        {
        }

        public StreamReaderNoAlloc(Stream stream, Encoding encoding, bool detectEncodingFromByteOrderMarks, int bufferSize, bool leaveOpen)
        {
        ...
        public StreamReaderNoAlloc(String path)
            : this(path, true)
        {
#if FEATURE_LEGACYNETCF
            if(CompatibilitySwitches.IsAppEarlierThanWindowsPhone8) {
        ...
        [ResourceExposure(ResourceScope.Machine)]
        [ResourceConsumption(ResourceScope.Machine)]
        public StreamReaderNoAlloc(String path, bool detectEncodingFromByteOrderMarks)
        ...
        [ResourceExposure(ResourceScope.Machine)]
        [ResourceConsumption(ResourceScope.Machine)]
        public StreamReaderNoAlloc(String path, Encoding encoding)
        ...
        [ResourceExposure(ResourceScope.Machine)]
        [ResourceConsumption(ResourceScope.Machine)]
        public StreamReaderNoAlloc(String path, Encoding encoding, bool detectEncodingFromByteOrderMarks)
        ...
        [ResourceExposure(ResourceScope.Machine)]
        [ResourceConsumption(ResourceScope.Machine)]
        public StreamReaderNoAlloc(String path, Encoding encoding, bool detectEncodingFromByteOrderMarks, int bufferSize)
        ...
        [ResourceExposure(ResourceScope.Machine)]
        [ResourceConsumption(ResourceScope.Machine)]
        internal StreamReaderNoAlloc(String path, Encoding encoding, bool detectEncodingFromByteOrderMarks, int bufferSize, bool checkHost)
        {
        ...
        public virtual bool ReadLine(StringBuilder sb)
        {
            if (sb == null)
            {
                return false;
            ...
            do
            {
                ...
                do
                {
                    char ch = charBuffer[currPos];
                    // Note the following common line feed chars:
                    // \n - UNIX   \r\n - DOS   \r - Mac
                    if (ch == '\r' || ch == '\n')
                    {
                        sb.Append(charBuffer, charPos, currPos - charPos);
                    ...
                    currPos++;
                } while (currPos < charLen);

                currPos = charLen - charPos;

                sb.Append(charBuffer, charPos, currPos);
            } while (ReadBuffer() > 0);
        ...
        private class NullStreamReader : StreamReaderNoAlloc
        {
