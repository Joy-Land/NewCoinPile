...
namespace OverdrawForURP
{
	public class OverdrawPass : ScriptableRenderPass
	{
		...
		public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
		{
			...
			{
                //Debug.Log("fzy ccc:" + camera.name + "   " + camera.GetHashCode() + "   " + isOffscreenDepthTexture);
                var camera = renderingData.cameraData.camera;
				if (isOpaque)
				{
						//Debug.Log("fzy camera:" + renderingData.cameraData.isSceneViewCamera + "   " + renderingData.cameraData.camera.name);
				...
				//if (isOpaque)
				//{
    //                //var s = drawSettings.sortingSettings;
    //                //Debug.Log("fzy ???:" + s.criteria + "  " + s.distanceMetric + "  " + s.customAxis + "  " + s.cameraPosition);
    //                //var ccc = drawSettings.overrideMaterial;
				//	//上面设置overridematerial会导致渲染顺序改变，具体原因不详细，用renderfeature的方式无法处理这个问题，只能是在urp源码中改给litshader多加个pass了
				//}
