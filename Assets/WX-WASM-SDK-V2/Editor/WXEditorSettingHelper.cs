...
namespace WeChatWASM
{
    ...
    public class WXSettingsHelper
    {
        ...
        public void OnBuildButtonGUI(EditorWindow window)
        {
            ...
            if (GUILayout.Button(new GUIContent("生成并转换(测试包)"), GUILayout.Width(130), GUILayout.Height(25)))
            {
            ...
            if (GUILayout.Button(new GUIContent("生成并转换(正式包)"), GUILayout.Width(130), GUILayout.Height(25)))
            {
        ...
        public void SetEditorStatus(bool isRelease)
        {
            if (isRelease)
            {
                BuildTargetGroup targetGroup = EditorUserBuildSettings.selectedBuildTargetGroup;
                Debug.Log(EditorUserBuildSettings.selectedBuildTargetGroup + "  " + EditorUserBuildSettings.activeBuildTarget);
                var definesStr = PlayerSettings.GetScriptingDefineSymbolsForGroup(targetGroup);
                var splitedDefines = definesStr.Split(';').ToList();
                splitedDefines = splitedDefines.Where(str => !str.Contains("JOYLAND_DEBUG")).ToList();
                PlayerSettings.SetScriptingDefineSymbolsForGroup(targetGroup, string.Join(";", splitedDefines));
                //设置stack trace
            ...
            else
            {
                ...
                foreach (var define in splitedDefines)
                {
                    if (define == "JOYLAND_DEBUG")
                    {
                        hasDefine = true;
                        break;
                    }
                ...
                if (hasDefine == false)
                {
                    splitedDefines.Add("JOYLAND_DEBUG");
                }
                PlayerSettings.SetScriptingDefineSymbolsForGroup(targetGroup, string.Join(";", splitedDefines));
            }
        }
